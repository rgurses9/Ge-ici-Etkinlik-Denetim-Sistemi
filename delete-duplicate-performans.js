/**
 * Bu script, Performans Özel Güvenlik şirketindeki mükerrer kayıtları temizler.
 * Her TC için SADECE İLK okutmayı tutar, 2. ve sonraki okutmaları siler.
 * 
 * KULLANIM:
 * node delete-duplicate-performans.js
 */
...
async function deleteDuplicatePerformansRecords() {
    const eventName = "13.02.2026 Galatasaray A.Ş. – İkas Eyüpspor Trendyol Süper Ligi Futbol Müsabakası";
    const companyName = "Performans Özel Güvenlik ve Koruma Hizmetleri Ltd. Şti.";

    console.log(`🔧 Performans Özel Güvenlik mükerrer kayıtları temizleniyor...\n`);
    ...
    const performansScans = allScans.filter(s =>
        s.eventId === event.id &&
        s.companyName === companyName
    );

    console.log(`📊 Performans Özel Güvenlik'te ${performansScans.length} okutma bulundu.`);

    // TC'ye göre grupla
    const tcGroups = {};
    performansScans.forEach(scan => {
    ...
    if (duplicateCount > 10) {
        console.log(`   ... ve ${duplicateCount - 10} mükerrer TC daha`);
    }

    console.log(`\n📊 Özet:`);
    console.log(`   Toplam okutma: ${performansScans.length}`);
    ...
    const updates = {
        ...
        [`companyCounts.Performans Özel Güvenlik ve Koruma Hizmetleri Ltd_ Şti_`]: increment(-duplicates.length)
    };
    ...
    console.log(`\n🎉 İşlem tamamlandı!`);
    console.log(`   Performans Özel Güvenlik: ${uniqueTCs} benzersiz kişi (mükerrerler temizlendi) ✅`);

    process.exit(0);
}

deleteDuplicatePerformansRecords().catch(err => {
